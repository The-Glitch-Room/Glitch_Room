  ...
  <div className="space-y-3">
    ...
    >
      <div className="flex items-center gap-2">
        ...
        >
          {passed ? "Solid submission — nice work" : "Needs more depth"}
        </span>
      </div>
...
const FixCreativeSpark = () => {
  const { id } = useParams();
  const challenge = creativeChallenges.find((item) => String(item.id) === id);
  ...
  const difficulty = challenge?.difficulty || challenge?.level || "Easy";
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const [pts, prev] = await Promise.all([
        fetchPoints(),
        challenge ? checkIfSolved(challenge.id, "spark") : null,
      ]);
      setPoints(pts);
  ...
  const handleSubmit = async () => {
    if (!userAnswer.trim()) {
      alert("Please describe your approach before submitting!");
      return;
    }
    ...
    const scenario = `Creative challenge: ${challenge.title}

Description: ${challenge.description || "N/A"}
${challenge.prompt ? `Prompt: ${challenge.prompt}\n` : ""}
${
  challenge.solution
    ? `A sample solution for reference (this is illustrative, not the only correct answer — do not penalize the user for taking a different creative direction):\n${challenge.solution}\n\n`
    : ""
}This is an open-ended creative/design challenge with no single correct answer. Evaluate the user's submission on originality, clarity, relevance to the prompt, and depth of thinking — not on matching the sample solution. Score strictly: a vague, generic, or off-topic answer should score low even if something was typed.`;
    ...
    try {
      ...
      if (isFirstPass) {
        ...
        await saveSubmission(challenge.id, "spark", userAnswer, earnablePoints);
        ...
        <div className="bg-[#0B0C10] flex flex-col min-h-0">
          <div className="shrink-0 flex items-center gap-1 px-2.5 sm:px-3 pt-2.5 sm:pt-3 border-b border-white/6 overflow-x-auto no-scrollbar">
            {[
              ...
              {
                id: "solution",
                label: solutionVisible ? "Sample" : "Sample 🔒",
            ...
            <AnimatePresence mode="wait">
              {leftTab === "description" && (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 text-xs text-gray-600 bg-white/[0.02] border border-white/5 rounded-xl px-3.5 py-2.5">
                    <Sparkles size={13} style={{ color: COLOR }} />
                    Open-ended — the AI grader scores originality and depth, not
                    a single "correct" answer.
                  </div>

                  {alreadySolved && (
                    <div className="mt-4 flex items-center gap-2 text-green-400 text-xs bg-green-500/5 border border-green-500/15 rounded-xl px-3.5 py-2.5">
                      <CheckCircle2 size={13} />
                      You've already earned points for this one — resubmitting
                      won't earn more.
                    </div>
                  )}
                </motion.div>
                ...
                >
                  {solutionVisible ? (
                    <div>
                      ...
                      <div className="bg-[#080810] border border-green-500/20 rounded-xl p-4">
                        <p className="text-gray-300 text-sm whitespace-pre-wrap leading-relaxed">
                          {challenge.solution ||
                            "Think creatively! There's no single right answer — focus on originality, clarity, and impact."}
                        </p>
                      </div>
                      <p className="text-gray-600 text-xs mt-3">
                        This is just one example — the AI grader accepts other
                        creative directions too.
                      </p>
                    </div>
                  ) : (
        ...
        <div className="bg-[#0B0C10] flex flex-col min-h-0">
          <div className="flex-1 min-h-0 flex flex-col p-4">
            ...
            <textarea
              ...
              disabled={evaluating}
              className="flex-1 w-full bg-[#080810] border border-white/8 rounded-xl p-4 text-gray-200 placeholder-gray-600 outline-none transition text-sm resize-none disabled:opacity-50 focus:ring-2"
            ...
            <div className="flex-1 overflow-y-auto p-4">
              {rightTab === "answer" && (
                <div className="text-xs text-gray-500 leading-relaxed space-y-2">
                  ...
                  <p>
                    This is open-ended: submissions are graded by AI on
                    originality, clarity, and relevance. You need a score of{" "}
                    <span className="text-white font-semibold">
                      {PASS_THRESHOLD}/10
                    </span>{" "}
...
export default FixCreativeSpark;
