...
import debugChallenges from "../data/debug_mode_challenges.json";
...
const FixBug = () => {
  const { id } = useParams();
  const challenge = debugChallenges.find((item) => String(item.id) === id);
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const [pts, prev] = await Promise.all([
        fetchPoints(),
        challenge ? checkIfSolved(challenge.id, "bug") : null,
      ]);
      setPoints(pts);
  ...
  const handleShowSolution = async () => {
    ...
    const next = await updatePoints(-5, `Hint used: ${challenge.title}`, "bug");
    setPoints(next);
...
${challenge.code ? `Buggy code given to the user:\n${challenge.code}\n\n` : ""}${
      challenge.solution
        ? `Reference correct solution (for grading only, do not reveal it verbatim in feedback):\n${challenge.solution}\n\n`
        : ""
    }Evaluate whether the user correctly identified the bug and proposed a valid fix. Score strictly — a vague, empty, or off-topic answer should score low even if something was typed.`;
    ...
    try {
      ...
      const alreadyPassedBefore = await hasPassedChallenge(challenge.id, "bug");
        ...
        <div className="bg-[#0B0C10] flex flex-col min-h-0">
          <div className="flex-1 min-h-0 flex flex-col p-4">
            ...
            <textarea
              ...
              placeholder="Explain the bug and your fix — you can include code inline. Be specific: a vague answer will score low."
            />
            <div className="flex flex-wrap sm:flex-nowrap items-center justify-between gap-3 mt-3">
              ...
              <motion.button
                ...
                style={{
                  background: `linear-gradient(135deg, ${COLOR}, #FF00C8)`,
                }}
              >
                {evaluating ? (
                  <>
          ...
          >
            ...
            >
              ...
              <p className="text-gray-400 text-sm mb-3">
                The AI grader confirmed your fix. Keep solving to level up.
              </p>
              <p
