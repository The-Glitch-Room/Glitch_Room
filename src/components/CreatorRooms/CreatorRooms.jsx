...
const CATEGORY_ACCENTS = {
  coding: { hex: "#00F0FF", glow: "rgba(0,240,255,0.22)", from: "from-cyan-950/80", label: "Coding & DSA" },
  design: { hex: "#FF00C8", glow: "rgba(255,0,200,0.22)", from: "from-pink-950/80", label: "Design & Sparks" },
  ai: { hex: "#2DD4BF", glow: "rgba(45,212,191,0.22)", from: "from-teal-950/80", label: "AI & Debug" },
  default: { hex: "#A855F7", glow: "rgba(168,85,247,0.22)", from: "from-purple-950/80", label: "Accountability" },
};

const getCategoryAccent = (category) => {
  const cat = (category || "").toLowerCase();
  if (cat.includes("coding") || cat.includes("dsa") || cat.includes("tech")) return CATEGORY_ACCENTS.coding;
  if (cat.includes("design") || cat.includes("spark") || cat.includes("creative")) return CATEGORY_ACCENTS.design;
...
const RoomCard = ({ room, isMember, onJoin, onEnter, joining }) => {
  ...
  return (
    ...
    >
      ...
      {/* TOP BANNER SECTION */}
      <div className={`relative h-28 w-full bg-gradient-to-b ${accent.from} via-[#100c1e] to-[#0c0c16] overflow-hidden border-b border-white/5 p-4 flex items-start justify-between`}>
      ...
      <div className="p-5 space-y-4 flex-1 flex flex-col justify-between relative">
        ...
        <div className="flex items-center gap-2 text-xs font-mono">
          <span className="flex-1 flex items-center gap-2 px-3 py-2 rounded-xl bg-white/[0.03] border border-white/5">
            ...
            <span className="text-white font-sans font-bold">{room.member_count || 1}</span>
