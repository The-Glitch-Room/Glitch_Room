...
// Page color: Cyan #00F0FF
const COLOR = "#00F0FF";

const GlitchesChallenges = () => {
  ...
  useEffect(() => {
    const fetchGlitches = async () => {
      ...
      if (!error) setGlitches(data || []);
      setLoading(false);
    };
    fetchGlitches();
  }, []);

  const categories = ["All", ...new Set(glitches.map((c) => c.category))];
  const filteredChallenges =
    selectedCategory === "All"
      ? glitches
      : glitches.filter((c) => c.category === selectedCategory);
    ...
    <div className="min-h-screen bg-[#0B0C10] text-white">
      ...
      <section className="relative pt-40 pb-16 px-6 overflow-hidden">
        ...
        <div className="relative z-10 max-w-3xl mx-auto">
          <PageHeading
            eyebrow="The Glitch Room"
            title="Glitch Challenges"
            subtitle="Spot the bug, fix the glitch, ship the fix. Real-world code scenarios across Web Dev, Machine Learning, and DSA."
            accent="cyan"
            size="xl"
          />
        </div>
      ...
      <div className="flex items-center justify-between gap-4 px-6 md:px-16 mb-10 flex-wrap">
        ...
        <p className="text-xs text-gray-500">
          ...
          glitch{filteredChallenges.length !== 1 ? "es" : ""}
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-24 text-gray-500 px-6">
          No glitch challenges yet — check back soon!
        </div>
      ) : (
        <motion.div
          layout
          ...
          {filteredChallenges.map((challenge, index) => {
            ...
            return (
              <motion.div
                ...
                whileHover={{ y: -4 }}
                className="group relative bg-[#0f0f14] border border-white/5 rounded-2xl p-5 flex flex-col justify-between transition-all duration-300 hover:border-[#00F0FF]/20 hover:bg-[#111118]"
              >
                {/* Number */}
                <div className="absolute top-4 right-4 text-xs font-bold text-white/10 group-hover:text-[#00F0FF]/30 transition-all">
                ...
                <div>
                  ...
                  <h2 className="text-white font-bold text-base leading-snug mb-3 group-hover:text-[#00F0FF] transition-colors pr-6">
                    {challenge.title}
                  </h2>
                ...
                <div className="flex items-center justify-between mt-auto">
                  ...
                  <button
                    onClick={() => navigate(`/glitch/${challenge.id}`)}
...
export default GlitchesChallenges;
